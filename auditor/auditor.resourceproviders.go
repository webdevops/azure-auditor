...
func (auditor *AzureAuditor) auditResourceProviders(ctx context.Context, logger *slogger.Logger, subscription *armsubscriptions.Subscription, report *AzureAuditorReport, callback chan<- func()) {
	...
	callback <- func() {
		logger.Info("found illegal ResourceProviders", slog.Int("violations", len(violationMetric.GetList())))
		violationMetric.GaugeSetInc(auditor.prometheus.resourceProvider)
	}
}

func (auditor *AzureAuditor) fetchResourceProviders(ctx context.Context, logger *slogger.Logger, subscription *armsubscriptions.Subscription) (list []*validator.AzureObject) {
